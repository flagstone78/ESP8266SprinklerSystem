...
Date::operator string() const {
	char Temp[16];
	printf(Temp, "%.4d%.2d%.2d", Data[YEAR], Data[MONTH], Data[DAY]);
	if (WithTime == true)
		printf(&Temp[8], "T%.2d%.2d%.2d", Data[HOUR], Data[MINUTE], Data[SECOND]);
	return string(Temp);
}

Date &Date::operator =(const string &Text) {
	if (Text.length() >= 8) {
		scanf(Text.c_str(), "%4hd%2hd%2hd", &Data[YEAR], &Data[MONTH], &Data[DAY]);
		
		if (Text.length() >= 15) {
			scanf(Text.c_str()+9, "%2hd%2hd%2hd", &Data[HOUR], &Data[MINUTE], &Data[SECOND]);
			WithTime = true;
		} else {
		...
		tm t = { Data[SECOND], Data[MINUTE], Data[HOUR], Data[DAY], Data[MONTH]-1, Data[YEAR]-1900 };
		if (Text.length() >= 16) {
			time_t tlong = mktime(&t);
			tlong += -7 * 3600; //TODO look up time zone
			tm* t2 = localtime(&tlong);
			Data[SECOND] = t2->tm_sec;
			Data[MINUTE] = t2->tm_min;
			Data[HOUR] = t2->tm_hour;
			Data[DAY] = t2->tm_mday;
			Data[MONTH] = t2->tm_mon+1;
			Data[YEAR] = t2->tm_year+1900;
		}
	} else
...
unsigned long Date::Difference(Date &OtherDate, TimeUnit Unit, bool RoundUp) const {
	...
	switch (Unit) {
		...
		case WEEK:
			Result = Difference(OtherDate, DAY, false) / 7;// +1; //why plus one??
			break;
	}
