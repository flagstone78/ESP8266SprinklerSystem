...
void ICalendar::LoadFromFile(string Line) {
		...
		switch (CurrentComponent) {
			...
			case VEVENT:
				if (Line.find("UID") == 0) {
					...
					GetTMFromString(NewEvent->DtStamp,GetProperty(Line));
				} else if (Line.find("DTSTART") == 0) {
					GetTMFromString(NewEvent->DtStart,GetProperty(Line));
				} else if (Line.find("DTEND") == 0) {
					GetTMFromString(NewEvent->DtEnd,GetProperty(Line));
					...
					if (NewEvent->RRule.Interval == 0)
						NewEvent->RRule.Interval = 1; //default to every occurence if interval is not specified
					NewEvent->RRule.Count = atoi(GetSubProperty(Line, "COUNT").c_str());
					NewEvent->RRule.Until = GetSubProperty(Line, "UNTIL");
					NewEvent->RRule.ByDay = ConvertByDayList(GetSubProperty(Line, "BYDAY"));
					...
					//TODO: dont add events that have passed and will not occur again
...
Event* ICalendar::Query::GetNextEvent(bool WithAlarm) {
	...
	if (RecurrentEvent == NULL) {
		for (; EventsIterator != Calendar->Events.end(); ++EventsIterator) {
			if ((*EventsIterator)->DtEnd.IsEmpty()) { //fill DTend if empty
